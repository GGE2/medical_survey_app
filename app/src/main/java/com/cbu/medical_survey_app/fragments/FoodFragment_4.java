...
public class FoodFragment_4 extends Fragment {

    private ButtonListener btl;

    private table1[] dooboos;
    private table2[] dooboos_radio;

    public FoodFragment_4() {
    }

    public FoodFragment_4(Context context) {
        btl = new ButtonListener(context);
        dooboos = new table1[6];
        dooboos_radio = new table2[6];
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle saveInstanceState) {
        ViewGroup vg = (ViewGroup) inflater.inflate(R.layout.food_frag_4, container, false);

        Button bt_food_4_prev = (Button) vg.findViewById(R.id.bt_food_4_prev);
        Button bt_food_4_next = (Button) vg.findViewById(R.id.bt_food_4_next);
        bt_food_4_prev.setOnClickListener(btl);
        bt_food_4_next.setOnClickListener(btl);
    ...
    private void initViews(ViewGroup vg) {
        ...
        for (int doobooID = 0; doobooID < dooboos_radio.length; doobooID++) {
            CheckBox[] cbs = new CheckBox[3];

            for (int ansID = 0; ansID < cbs.length; ansID++) {
                cbs[ansID] = vg.findViewById(getResId(vg, "radio_dooboo" + (doobooID + 1) + "_ans" + (ansID + 1)));
            }
            dooboos_radio[doobooID] = new table2(cbs);
        }

        for (int doobooID = 0; doobooID < dooboos.length; doobooID++) {
            CheckBox[] cbs = new CheckBox[9];

            for (int ansID = 0; ansID < cbs.length; ansID++) {
                cbs[ansID] = vg.findViewById(getResId(vg, "check_dooboo" + (doobooID + 1) + "_ans" + (ansID + 1)));
            }
            dooboos[doobooID] = new table1(cbs, doobooID);
        }
    }
    ...
    private class table1 {
        ...
        public table1(CheckBox[] cbs, int index) {
            ...
            if(checked == 0 || checked == -1) {
                for (CheckBox cb : dooboos_radio[index].check_bts){
                    disabledChecked(cb);
                }
            }
            else{
